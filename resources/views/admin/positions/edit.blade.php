                    ...
                    <form action="{{ route('admin.positions.update', $position) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select class="form-select @error('election_ids') is-invalid @enderror" 
                                ...
                                @foreach($elections as $election)
                                    <option value="{{ $election->id }}" 
                                            {{ in_array($election->id, old('election_ids', $position->elections->pluck('id')->toArray())) ? 'selected' : '' }}>
                                        {{ $election->title }}
                                    </option>
                                @endforeach
                            </select>
                            @error('election_ids')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <small class="text-muted">Hold Ctrl/Cmd to select multiple elections. Leave empty to unassign from all elections.</small>
                            @if($position->candidates->count() > 0)
                                <small class="text-warning">
                                    <i class="bi bi-exclamation-triangle"></i> 
                                    Warning: This position has {{ $position->candidates->count() }} candidate(s). 
                                    Changing the election assignments may cause issues.
                                </small>
                            @endif
                        </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-2">
                                ...
                                <span id="preview-election" class="float-end badge bg-success">{{ $position->elections->pluck('title')->join(', ') ?: 'Not assigned' }}</span>
                            </div>
                            <div class="mb-2">
    ...
    <x-slot name="scripts">
        <script>
            ...
            document.getElementById('election_ids').addEventListener('change', function() {
                ...
                document.getElementById('preview-election').textContent = selectedOptions.length > 0 ? selectedOptions.join(', ') : 'Not assigned';
            });
